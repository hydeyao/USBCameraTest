...
void VideoWidgetFrame::setDrawType(int type)
{
	mDrawType = type;
}

void VideoWidgetFrame::paintOnVideo()
{
	switch (mDrawType)
	{
	case PAINT_CLEAR:
		break;
	case PAINT_CROSS_LINE:
		draw_cross_line(); break;
	case PAINT_RECTS:
		draw_rects(); break;
	case PAINT_CROSS_LINE | PAINT_RECTS:
		draw_cross_line();
		draw_rects();
		break;
	default:
...
GLVideoWidget::GLVideoWidget(QWidget* parent) : VideoWidgetFrame(parent)
{
    m_pos = QPointF(0, 0);
...
void GLVideoWidget::repaint(AVFrame* frame)
{
...
void GLVideoWidget::draw_rects()
{
    ...
    {
        auto [ltx, lty, rtx, rty, rbx, rby, lbx, lby] = roi->pos();
...
void GLVideoWidget::initCrossLineShader()
{
    if (!m_paintProg)
    {
    ...
    m_paintProg->setUniformValue("ourColor", QVector4D(0.2,0.0,0.0,1.0));

    m_paintProg->enableAttributeArray(attr);
    m_paintProg->release();
}

NormalVideoWidget::NormalVideoWidget(QWidget* par) :VideoWidgetFrame(par)
{
}

NormalVideoWidget::~NormalVideoWidget()
{
}
...
void NormalVideoWidget::paintEvent(QPaintEvent* e)
{
...
void NormalVideoWidget::draw_cross_line()
{
    QPainter painter(this);
